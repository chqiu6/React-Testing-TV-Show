...
test("Episodes list shows list of episodes when rendered with episode data", () => {
  // Arrange render component before api call 
  const { queryAllByText, rerender } = render(
    <Episodes episodes={[]} />
  );
  const emptyEpisodes = queryAllByText(/episode/i);
  expect(emptyEpisodes).toHaveLength(0);

  // Act rerenders component with api call 
  rerender(<Episodes episodes={data} />);
  const reloadedEpisodes = queryAllByText(/episode/i);

  // Assert expects a length of 8 episodes
  expect(reloadedEpisodes).toHaveLength(8);
